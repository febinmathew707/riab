...
function BasicTable($header,$data)
{
   ...
   ////////////////////////////////////  HEADING /////////////////////////////
	...
	$this->Cell(188,6,"PERFORMANCE Of PSUs UNDER INDUSTRIES DEPARTMENT ( ".$month->getMonth($tmth)." "
	.substr($tyear,2,2)." - ". substr($tyear,7)." )",0,0,'C');
	$this->ln();
	$this->SetFont('Arial','B',12);
	$this->Cell(188,6,"UNITS WITH INCREASE IN LOSSES",0,0,'C');
	$this->ln();
	
	$this->SetFont('Arial','',8);
	$this->Cell(188,6,"(Rs. in Lakhs)",0,0,'R');
	$this->ln();
	$this->SetFont('Arial','',8);
	/// FIRST LINE
    $this->Cell(10,5,"","L,T,R",0,'C');
    $this->Cell(60,5,"","L,T,R",0,'C');
    $this->Cell(59,5,"TURNOVER","L,T,R",0,'C');
    $this->Cell(59,5,"PROFIT/LOSS","L,T,R",1,'C');
    
    ///////////////// SECOND LINE
	$this->Cell(10,4,"Sl.No","L,R",0,'C');
    $this->Cell(60,4,"COMPANY","L,R",0,'C');
    $this->SetFont('Arial','',6);
    $this->Cell(59,4,"(without taxes and duties)","L,R,B",0,'C');
    $this->Cell(44,4,"","L,B",0,'C');
    $this->SetFont('Arial','',8);
    $this->Cell(15,4,"","R",1,'C');
    ////////// THIRD LINE //////////////
    $this->SetFont('Arial','',8);
    $this->Cell(10,4,"","L,R",0,'C');
    $this->Cell(60,4,"","L,R",0,'C');
    $this->Cell(22,4,substr($fmonth,0,3)." ".substr($f-1,2,2)." - ".substr($tmonth,0,3)." ".substr($t-1,2,2),"R",0,'C');
    $this->Cell(22,4,substr($fmonth,0,3)." ".substr($f,2,2)." - ".substr($tmonth,0,3)." ".substr($t,2,2),"R",0,'C');
    
    $this->Cell(15,4,"DIFF","L,R,T",0,'C');
    
   $this->Cell(22,4,substr($fmonth,0,3)." ".substr($f-1,2,2)." - ".substr($tmonth,0,3)." ".substr($t-1,2,2),"R",0,'C');
    $this->Cell(22,4,substr($fmonth,0,3)." ".substr($f,2,2)." - ".substr($tmonth,0,3)." ".substr($t,2,2),"R",0,'C');
    $this->Cell(15,4,"DIFF","L,R,T",1,'C');
    /////////////////// FOURHT LINE ///////////////////////
     $this->SetFont('Arial','',8);
    $this->Cell(10,4,"","L,R,B",0,'C');
    $this->Cell(60,4,"","L,R,B",0,'C');
    $this->Cell(22,4,"","L,R,B",0,'C');
    $this->Cell(22,4,"","L,R,B",0,'C');
    $this->Cell(15,4,"ERENCE","L,R,B",0,'C');
    $this->Cell(22,4,"","L,R,B",0,'C');
    $this->Cell(22,4,"","L,R,B",0,'C');  
    $this->Cell(15,4,"ERENCE","L,R,B",1,'C');
    ///////////// BODY OF REPORT ///////////////////
     $slno=1;
...
mysql_query($strsql); 
   ...
   $strsql="select company_id from 
			emp_monthly_report where fin_year=".$t." and month=".$tmth." and cumulative_net_profit<0 and status='A'    order by cumulative_net_profit desc";
    ...
    while($row=mysql_fetch_array($result))
    {
		...
		//$this->Cell(60,4,$sqlcheck,"L,R,B",1,'C');
		...
		$sqlcheck1="SELECT status from emp_monthly_report WHERE company_id=".$row["company_id"]." and 
				 fin_year=".$fyear." and month=".$tmth." order by company_id";
		...
		if($rowcheck[0]=="A")
		{
			...
			if($pandl<$prepandl && $pandl<0 && $prepandl<0)
			{
				$diff=abs($pandl)-abs($prepandl);
				$diffTurn=abs($turn)-abs($preturn);	
				...
				if(strlen($rowcom["company_name"])>37)
		       {
				$len=37;
				$pos=37;
				$no=0;
				$this->Cell(10,4,$slno,"L,R",0,'C');
				$this->Cell(60,4,substr($rowcom["company_name"],0,37),"R",0,'L');
				...
				$this->ln();
				$this->Cell(10,4,"","L,R,B",0,'C');
				$this->Cell(60,4,substr($rowcom["company_name"],$pos,$len),"R,B",0,'L');
			...
			else
			{
				...
				$this->Cell(60,4,substr($rowcom["company_name"],0,37),"R,B",0,'L');
			}		
					//$this->Cell(10,4,$slno,"L,R,B",0,'C');
					//$this->Cell(60,4,$rowcom["company_name"],"R,B",0,'L');
				
		    	$this->Cell(22,4,number_format($preturn,2,'.',''),"L,R,B",0,'R');
	    		$this->Cell(22,4,number_format($turn,2,'.',''),"L,R,B",0,'R');
	    		$this->Cell(15,4,$diffTurn,"L,B,R",0,'R');
