...
function BasicTable($header,$data)
{
   ...
   */
   ...
   $this->SetFont('Arial','B',12);
    ...
    $this->Cell(90,7,"","T",0,'C');
    $this->Cell(199,7,"Performance During ".$month->getMonth($fmth). " ". $fin1." - "
    ...
    while($row=mysql_fetch_array($result))
    {
		...
		//$this->Cell(110,4,$row["company_name"],"R,B",0,'C');   
			...
			$this->Cell(90,4,$row["company_name"],"R,B",0,'L');
	    ...
	    $strsql="SELECT cumulative_rowmaterial,cumulative_power_fuel,cumulative_other_variablecost from emp_variable_expenses where 
		company_id=".$row["company_id"]." 	and rpt_date='".$tdate."' and status='A'";
		$resultsub=mysql_query($strsql);
		$rowsub=mysql_fetch_row($resultsub);
	    $this->Cell(17,4,number_format($rowsub[0],2,'.',''),"L,R,B",0,'R'); 
   	    $this->Cell(17,4,number_format($rowsub[1],2,'.',''),"L,R,B",0,'R');
		$this->Cell(17,4,number_format($rowsub[2],2,'.',''),"L,R,B",0,'R');
	    $totCumulativeRowmaterial+=$rowsub[0];
		$totCumulativePowerFuel+=$rowsub[1];
	    $totCumulativeOtherVariableCost+=$rowsub[2];
	...
	$this->Cell(17,4,number_format($totCumulativePowerFuel,2,'.',''),"L,R,B",0,'R');
