        ...
        <td width="790" height="31" class="txtleft">
			<table border="1" cellpadding="3" cellspacing="3" style="border-collapse: collapse" bordercolor="#111111" width="100%" height="64">       
				<tr>
					...
					<td width="10%" height="40" class="txtcenter1" bgcolor="#D7D7D7" rowspan="2" align="centre"><b>Achievable capacity per annum</b><br></td>
					<td width="20%" height="40" class="txtcenter1" bgcolor="#D7D7D7" colspan="2" align="center"><b>Production (Qty)</b></td>
					<td width="20%" height="40" class="txtcenter1" bgcolor="#D7D7D7" colspan="2" align="center"><b>Capacity Utilization(%)</b></td>		
					<td width="19%" height="40" class="txtcenter1" bgcolor="#D7D7D7" colspan="2" align="center"><b>Sales (Qty)</b></td>
				</tr>
				<tr>
					<td width="9%"  height="40" class="txtcenter1" bgcolor="#D7D7D7" align="center"><b>For the Month</b></td>            
					<td width="11%" height="40" class="txtcenter1" bgcolor="#D7D7D7" align="center"><b>Cumulative for the year</b></td>
					<td width="9%"  height="40" class="txtcenter1" bgcolor="#D7D7D7" align="center"><b>For the Month</b></td>            
					<td width="11%" height="40" class="txtcenter1" bgcolor="#D7D7D7" align="center"><b>Cumulative for the year</b></td>
					<td width="8%"  height="40" class="txtcenter1" bgcolor="#D7D7D7" align="center"><b>For the Month</b></td>            
					<td width="8%"  height="40" class="txtcenter1" bgcolor="#D7D7D7" align="center"><b>Cumulative for the year</b></td>            
				</tr>  
				<?php
				...
				<tr>
					<td width="16% height="23" class="txtleft" bgcolor="#E2E0E0">
						<?php echo $rowcapacity["item_name"]."<input type=hidden id='txtItCode' name='txtItCode' size=10 value=".$rowcapacity["item_code"].">";
						?>
					</td>
					...
					<td width="9%" height="23" class="txtcenter1" >
						<?php 
							$sql="select * from emp_monthly_capacity where company_id=".$companyid." and fin_year=".$year."  and month=".$month." and it_code=".$rowcapacity["item_code"]."";
							$resultPro=mysql_query($sql);
							$rowPro=mysql_fetch_assoc($resultPro); 
							echo 	"<input type=text id='txtProdqtyFormonth' name='txtProdqtyFormonth' size=10 
					...
					<td width="8%" height="40" class="txtcenter1" align="center">
						<?php 
							if ($rowcapacity["ach_capacity"] > 0) {
								$Cuformonth = ($rowPro["prod_qty_formonth"] / ($rowcapacity["ach_capacity"] / 12))*100;
							} else {
								if ($rowcapacity["inst_capacity"] >0) {
									$Cuformonth = ($rowPro["prod_qty_formonth"] / ($rowcapacity["inst_capacity"] / 12))*100;
								} else {
									$Cuformonth =0;
								}
							}
							echo round($Cuformonth,2)."%";
						?>
					</td>
					<td width="11%" height="40" class="txtcenter1" align="center">
					<?php 
							...
							if ($rowcapacity["ach_capacity"] > 0) {
								$Cucum = ($rowPro["prod_qty_cum"] / (($rowcapacity["ach_capacity"] / 12)*$nm))*100;
							} else {
								if ($rowcapacity["inst_capacity"] >0) {
									$Cucum = ($rowPro["prod_qty_cum"] / (($rowcapacity["inst_capacity"] / 12)*$nm))*100;
								} else {
									$Cucum = 0;
								}
							}
...
<!--
 ...
 -->
          <tr>
            <td width="44%" class="txtleft">b) Value of Production - VOP</td>
          ...
          <tr>
            <td width="100%" class="txtleft" colspan="4" bgcolor="#D7D7D7"><b>c) Variable Expenses</b></td>
          </tr>
          <?php
          ...
          <?php //RAW MATERIAL CONSUMPTION ******************//; ?>
          
          <tr>
            <td width="44%" class="txtleft">&nbsp;&nbsp;&nbsp; i) Raw-material cost(% to VOP)
            </td>
            <td width="20%" class="txtleft">
                <?php
				if (($rowbud["sales"]+$rowbud["stock"])==0) {
					echo number_format($rowbud["current_rowmaterial"],2,'.','');
				} else {
					$perBudRM = ($rowbud["current_rowmaterial"]/($rowbud["sales"]+$rowbud["stock"]))*100;
					echo number_format($rowbud["current_rowmaterial"],2,'.','')."  (".number_format($perBudRM,2,'.','')."%)";}
	            ?>
       		</td>
       		
            <td width="20%" class="txtleft">
                <?php
				if (($rowPro["actual_sale"]+$rowPro["actual_stock"])==0) {
					echo number_format($rowVar['actual_rowmaterial'],2,'.','');
				} else {
					$perCurRM = ($rowVar['actual_rowmaterial']/($rowPro["actual_sale"]+$rowPro["actual_stock"]))*100;
					echo number_format($rowVar['actual_rowmaterial'],2,'.','')."  (".number_format($perCurRM,2,'.','')."%)";}
	            ?>
       		</td>

            <td width="20%" class="txtleft">
                <?php
				if (($rowPro["cumulative_sale"]+$rowPro["cumulative_stock"])==0) {
					echo number_format($rowVar['cumulative_rowmaterial'],2,'.','');
				} else {
						$perCumRM = ($rowVar['cumulative_rowmaterial']/($rowPro["cumulative_sale"]+$rowPro["cumulative_stock"]))*100;
						echo number_format($rowVar['cumulative_rowmaterial'],2,'.','')."  (".number_format($perCumRM,2,'.','')."%)";
				}
			    ?>
			</td>
          </tr>
          
          <?php //POWER AND FUEL CONSUMPTION ******************//; ?>
          
            <tr>
            <td width="44%" class="txtleft">&nbsp;&nbsp;&nbsp; ii) Power &amp; fuel(% to VOP)
            </td>
            <td width="20%" class="txtleft">
                <?php
					if (($rowbud["sales"]+$rowbud["stock"])==0) {
						echo number_format($rowbud["current_power_fuel"],2,'.','');					
					} else 	{
								$perBudPF = ($rowbud['current_power_fuel']/($rowbud["sales"]+$rowbud["stock"]))*100;
								echo number_format($rowbud["current_power_fuel"],2,'.','')."  (".number_format($perBudPF,2,'.','')."%)";
							}
	            ?>
       	    </td>
            ...
            <td width="20%" class="txtleft">
                <?php
					if (($rowPro["actual_sale"]+$rowPro["actual_stock"]) == 0) {
						echo number_format($rowVar['actual_power_fuel'],2,'.','');
					} else 	{
								$perCurPF = ($rowVar['actual_power_fuel']/($rowPro["actual_sale"]+$rowPro["actual_stock"]))*100;
								echo number_format($rowVar['actual_power_fuel'],2,'.','')."  (".number_format($perCurPF,2,'.','')."%)";
							}
				?>
     	    </td>
            ...
            <td width="20%" class="txtleft">
                <?php
					IF (($rowPro["cumulative_sale"]+$rowPro["cumulative_stock"])==0) {
						echo number_format($rowVar['cumulative_power_fuel'],2,'.','');
					} else 	{
						
							$perCumPF = ($rowVar['cumulative_power_fuel']/($rowPro["cumulative_sale"]+$rowPro["cumulative_stock"]))*100;
							echo number_format($rowVar['cumulative_power_fuel'],2,'.','')."  (".number_format($perCumPF,2,'.','')."%)";
							}
		        ?>
             </td>
          </tr>
          ...
          <tr>
			...
			value=".number_format($rowbud['current_other_variablecost'],2,'.','')." onblur='JavaScript:calcProfirability()' readonly>";
			?></td>
            <td width="19%" class="txtcenter1">
             <?php echo"
          ...
          <tr>
            <td width="100%" class="txtleft" colspan="4" bgcolor="#D7D7D7">d) <b>Fixed Expenses</b></td>
          </tr>
          <?php
          ...
          <?php //EMPLOYEE COST ******************//; ?>
            ...
            <tr>
            <td width="44%" class="txtleft">&nbsp;&nbsp;&nbsp; i) Employee Cost(% to VOP) </td>

            <td width="20%" class="txtleft">
                <?php
				if (($rowbud["sales"]+$rowbud["stock"])==0) {
					echo number_format($rowbud["current_employee_cost"],2,'.','');
				} else {
					$perBudPF = ($rowbud['current_employee_cost']/($rowbud["sales"]+$rowbud["stock"]))*100;
					echo number_format($rowbud["current_employee_cost"],2,'.','')."  (".number_format($perBudPF,2,'.','')."%)";
				}
	            ?>
       	    </td>
            ...
            <td width="20%" class="txtleft">
                <?php
				if (($rowPro["actual_sale"]+$rowPro["actual_stock"])==0){
					echo number_format($rowFix['actual_employee_cost'],2,'.','');
				} else {
					$perCurPF = ($rowFix['actual_employee_cost']/($rowPro["actual_sale"]+$rowPro["actual_stock"]))*100;
					echo number_format($rowFix['actual_employee_cost'],2,'.','')."  (".number_format($perCurPF,2,'.','')."%)";
				}
	            ?>
     	    </td>
            ...
            <td width="20%" class="txtleft">
                <?php
                if (($rowPro["cumulative_sale"]+$rowPro["cumulative_stock"]) ==0 ) {
					echo number_format($rowFix['cumulative_employee_cost'],2,'.','');
				} else 	{
						$perCumPF = ($rowFix['cumulative_employee_cost']/($rowPro["cumulative_sale"]+$rowPro["cumulative_stock"]))*100;
						echo number_format($rowFix['cumulative_employee_cost'],2,'.','')."  (".number_format($perCumPF,2,'.','')."%)";
						}
		        ?>
             </td>
          </tr>
          ...
          <tr>
            ...
            <input type=text name=txtCurrentOtherExpense id=txtCurrentOtherExpense
			 class=txtboxwhite size=25 
			value=".number_format($rowbud['current_other_expense'],2,'.','')." onblur='JavaScript:calcProfirability()' readonly>";
			?>
			</td>
            <td width="19%" class="txtcenter1">
            <?php echo"
          ...
          <tr>
            <td width="44%" class="txtleft">e) Total Cost (c+d)</td>
            <td width="20%" class="txtcenter1" bgcolor="#D7D7D7">
            <?php 
          ...
          <tr>
            <td width="44%" class="txtleft">f) Profit Before INT .DEPR.&amp; Taxes 
            (b-e)</td>
            <td width="20%" class="txtcenter1" bgcolor="#D7D7D7">
            <?php 
          ...
          <tr>
            <td width="100%" class="txtleft" colspan="4" bgcolor="#D7D7D7">g)<b> Financial Charges on</b></td>
          </tr>
          ...
          <tr>
            ...
            <input type=text name=txtCurrentLongTermLoan id=txtCurrentLongTermLoan
			 class=txtboxwhite size=25 
			value=".number_format($rowbud['current_longterm_loan'],2,'.','')." onblur='JavaScript:calcProfirability()' readonly>";
			?>
			</td>
            <td width="19%" class="txtcenter1">
			<?php echo"
          ...
          <tr>
            ...
            <input type=text name=txtCurrentWorkingTermLoan id=txtCurrentWorkingTermLoan
			 class=txtboxwhite size=25 
			value=".number_format($rowbud['current_workingterm_loan'],2,'.','')." onblur='JavaScript:calcProfirability()' readonly>";
			?>
			</td>
            <td width="19%" class="txtcenter1">
			<?php echo"
          ...
          ?>
          <tr>
            <td width="44%" class="txtleft">h) Profit Before DEPR. &amp; Taxes (f-g)</td>
            <td width="20%" class="txtcenter1">
            <?php 
          ...
          <tr>
            <td width="44%" class="txtleft">i) Other Income</td>
            <td width="20%" class="txtcenter1">
            <?php echo"
            <input type=text name=txtCurrentOtherIncome id=txtCurrentOtherIncome
			 class=txtboxwhite size=25 
			value=".number_format($rowbud['current_other_income'],2,'.','')." readonly>";
			?>
			</td>
            <td width="19%" class="txtcenter1">
			<?php echo"
          ...
          <tr>
            <td width="44%" class="txtleft">j) Cash Profit/Loss (h+i)</td>
            <td width="20%" class="txtcenter1" bgcolor="#D7D7D7">
            <?php 
          ...
          <tr>
            <td width="44%" class="txtleft">k) Depreciation</td>
            <td width="20%" class="txtcenter1">
            <?php echo"
            <input type=text name=txtCurrentDepreciation id=txtCurrentDepreciation
			 class=txtboxwhite size=25 
			value=".number_format($rowbud['current_depr'],2,'.','')." readonly>";
			?>
			</td>
            <td width="19%" class="txtcenter1">
             <?php echo"
          ...
          <tr>
            <td width="44%" class="txtleft">l)Net Profit/Loss (j-k) (PBT)</td>
            <td width="20%" class="txtcenter1" bgcolor="#D7D7D7">
            <?php 
